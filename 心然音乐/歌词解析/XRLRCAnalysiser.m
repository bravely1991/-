...
@implementation XRLRCAnalysiser
...
{
    // 获取到歌词文件的路径
    NSString *path = [[NSBundle mainBundle] pathForResource:fileName ofType:@"lrc"];
    ...
    for (NSString *timeAndText in timeAndTextArray) {
        ...
        // 创建歌词数据模型并且赋值
        XRLRCModel *lrcModel = [[XRLRCModel alloc] init];
        // 歌词时间
        lrcModel.beginTime = [lrcModel getTimeIntervalWithFormatTime:beginTimeStr];
        // 歌词内容
    ...
    for (NSInteger i = 0; i < count; i ++) {
        ...
        XRLRCModel *nowModel = lrcModels[i];
        XRLRCModel *nextModel = lrcModels[i + 1];
...
+ (NSInteger)getRowWithCurrentTime:(NSTimeInterval)currentTime lrcModels:(NSArray<XRLRCModel *> *)lrcModels
{
    ...
    NSInteger count = lrcModels.count;
    for (NSInteger i = 0; i < count; i ++) {
        XRLRCModel *lrcModel = lrcModels[i];
